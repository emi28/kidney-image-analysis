     ...
     for bb = 1:length(Folder)
           dirFolder = dir('*.jpg');                 %get the selected file data
             ...
             for iFile = 1:numel(fileNames)            %loop over the file names
                filename = sprintf('%02d.jpg',iFile);  %make the new name
               % movefile(fileNames{iFile},filename);   %rename the file
                movefile fileNames filename 
             end
...
for aa = 1:(length(dir)-6)   %3 is from 2 dots + a function 
