...
%              the figure
                   ...
                   if m_line~=0 %if the slope is not 0 (the line is not vertical...)
                       for index_new = 1:max_x
                           ...
                           y_range(isnan(y_range))=0; %gets rid of NaN values by setting them = to 0
                           y_range(isinf(y_range))=0; %gets rid of inf values by setting them = to 0
                           ...
                           z = w(:,1); %creates an array of intensity values 
                               if z(index_new)~=0 %if the intensity value of the coordinate is not 0 (black), then plot a red dot 
                                  ...
                                  hold on 
                                  plot(mid_x_test(1:n:end),mid_y_test(1:n:end), '*') %plots a star at the midpoint
                               else
                               end
                       end
                       ...
                       for index_new = 1:max_x
                           ...
                           x_vertical_range = repmat(mid_x_test_repeat,1,128); %creates an array of the same x value that is 512 values long
                           y_vertical_range = [1:512];
                           
                           y_vertical_range(isnan(y_vertical_range))=0; 
                           y_vertical_range(isinf(y_vertical_range))=0;
                           
                           w_vertical = improfile(simul_hypo, x_vertical_range, y_vertical_range,max_x);
                           z_vertical = w_vertical(:,1);
                                if z_vertical(index_new)~=0 %if the intensity value of the coordinate is not 0 (black), then plot a red dot 
                                  plot(x_vertical_range(index_new), y_vertical_range(index_new),'-.ro')
                                  hold on
